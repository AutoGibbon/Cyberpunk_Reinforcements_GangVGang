...
public class GRReinforcementSystem extends ScriptableSystem {
    ...
    private final func OnPlayerAttach(request: ref<PlayerAttachRequest>) -> Void { 
        let theGame = GetGameInstance(); // I just lost it
    ...
    private func ReinforcementsChecksCall(puppet: ref<ScriptedPuppet>, target: ref<GameObject>) -> Bool {
        ...
        if GameInstance.GetBlackboardSystem(gi).GetLocalInstanced(player.GetEntityID(), GetAllBlackboardDefs().PlayerStateMachine).GetInt(GetAllBlackboardDefs().PlayerStateMachine.SceneTier) >1{
            return false;
        };
        ...
        let nonoZones = [
            "LittleChina_Afterlife", "Dogtown_Akebono", "Northside_All_Foods", "CorpoPlaza_ArasakaTowerAtrium", "CorpoPlaza_ArasakaTowerCEOFloor", "CorpoPlaza_ArasakaTowerJenkins", "CorpoPlaza_ArasakaTowerJungle",
            "CorpoPlaza_ArasakaTowerLobby", "CorpoPlaza_ArasakaTowerNest", "CorpoPlaza_ArasakaTowerSaburoOffice", "CorpoPlaza_ArasakaTowerUnlistedFloors", "CorpoPlaza_ArasakaTowerUpperAtrium", "ArasakaWaterfront",
            "NorthOaks_Arasaka_Estate", "CharterHill_AuCabanon", "Coastview_BattysHotel", "Dogtown_Brooklyn", "Northside_CleanCut", "JapanTown_Clouds", "CorpoPlaza_Apartment", "Dogtown_Cynosure", "Vista_del_Rey_Delamain",
            "Dogtown_Expo", "JapanTown_FourthWallBdStudio", "RanchoCoronado_GunORama", "Dogtown_Hideout", "JapanTown_HiromisApartment", "NorthOaks_Kerry_Estate", "Kabuki_LizziesBar", "JapanTown_MegabuildingH8",
            "MorroRock", "Badlands_Spaceport", "MorroRock_NCX", "Northside_Apartment", "Kabuki_NoTellMotel", "LittleChina_Q101Cyberspace", "Northside_Totentaz", "LittleChina_VApartment", "Coastview_VDBChapel",
            "Coastview_VDBMaglev", "JapanTown_VR_Tutorial", "Northside_WNS"
        ];
    ...
    public func GetFactionHandler(puppet: ref<ScriptedPuppet>) -> ref<GRGangHandler> {
            let affiliation = TweakDBInterface.GetCharacterRecord(puppet.GetRecordID()).Affiliation().Type();
            switch affiliation {
